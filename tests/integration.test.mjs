...
test('full seeded backend workflow and permission gates',async()=>{
 await check('setup',{mode:'demo'});assert.equal(sqlite.prepare('SELECT count(*) n FROM students').get().n,1000);assert.equal(sqlite.prepare('SELECT count(*) n FROM tasks').get().n,1000);
 ...
 await check('account_request',{id:'REQ1',student_id:'S10001',task_bank_id:'TB-DM-1',job_profile:'Digital marketing specialist',gig_number:1});
 ...
 await check('account_request',{id:'REQ2',student_id:'S10002',task_bank_id:'TB-DM-1',job_profile:'Digital marketing specialist',gig_number:1});r=await post('allocate',{request:'REQ2',account:'ACC-102',task_fit:true});assert.match(r.error,/unavailable|assigned/);
...
test('controlled platforms and separately approved FX applications are enforced',async()=>{
 current={id:'owner',email:'owner@example.com'};
 assert.throws(()=>sqlite.prepare("INSERT INTO accounts(id,platform,label,status,credits) VALUES('BAD-PLATFORM','Fiverr','Invalid','Available',10)").run(),/platform/);
 sqlite.prepare('INSERT INTO attachments VALUES(?,?,?,?,?,?,?,?,?)').run('PROOF-FX','S10003','key-fx','fx.png','image/png',100,'hash-fx','owner',new Date().toISOString());
 await check('gig',{id:'GIG-FX',student_id:'S10003',platform:'Fiverr',title:'Converted service',value:300,currency:'EGP',order_ref:'fx-order-1',due:'2027-01-01T00:00:00Z'});
 for(const status of ['Gig Opened','Work Submitted','Delivered','Paid'])await check('gig_transition',{id:'GIG-FX',status,proof_id:'PROOF-FX',occurred_at:new Date().toISOString()});
 sqlite.prepare("INSERT INTO evidence(id,student_id,gig_id,proof_id,source,status,rejections,recorder,stage_at,created_at,policy_id) VALUES(?,?,?,?,?,'Accepted',0,?,?,?,?)").run('EV-FX','S10003','GIG-FX','PROOF-FX','Platform','quality-login',new Date().toISOString(),new Date().toISOString(),'R5-v1');
 await check('fx_rate',{id:'FX-EGP-1',currency:'EGP',usd_rate:0.02,effective_date:'2026-01-01',source:'Approved finance bulletin'});
 assert.match((await post('fx_rate_approve',{id:'FX-EGP-1',reason:'Self approval'})).error,/different/);
 current={id:'approver',email:'approver@example.com'};await check('fx_rate_approve',{id:'FX-EGP-1',reason:'Checked against the approved finance bulletin'});await check('fx_apply',{gig_id:'GIG-FX',fx_rate_id:'FX-EGP-1'});
 assert.equal(sqlite.prepare("SELECT usd_value FROM gig_fx_applications WHERE gig_id='GIG-FX'").get().usd_value,6);
 assert.equal(sqlite.prepare("SELECT result FROM graduation_ledger WHERE student_id='S10003' ORDER BY rowid DESC LIMIT 1").get().result,'1/3');
 assert.throws(()=>sqlite.prepare("UPDATE fx_rates SET usd_rate=1 WHERE id='FX-EGP-1'").run(),/immutable/);
});
...
test('global search is bounded to the signed-in staff scope',async()=>{
 const api=await route('search');current={id:'coordinator',email:'staff-sara@example.invalid'};let result=await(await api.GET(new Request('https://test.local/api/search?q=S10001'))).json();assert.ok(result.results.some(x=>x.type==='student'&&x.id==='S10001'));
 current={id:'other',email:'staff-omar@example.invalid'};result=await(await api.GET(new Request('https://test.local/api/search?q=S10001'))).json();assert.ok(!result.results.some(x=>x.type==='student'&&x.id==='S10001'));
});
test('retention policy records approved periods without deleting data',async()=>{
 const api=await route('retention');current={id:'owner',email:'owner@example.com'};const before=sqlite.prepare('SELECT count(*) n FROM attachments').get().n;let result=await(await api.POST(new Request('https://test.local/api/retention',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({scope:'attachments',retention_action:'archive',days:365,authority:'Approved test policy',reason:'Exercise controlled retention configuration'})}))).json();assert.equal(result.ok,true);result=await(await api.GET()).json();assert.ok(result.configurations.some(x=>x.key==='retention:attachments'));assert.equal(sqlite.prepare('SELECT count(*) n FROM attachments').get().n,before);
});
