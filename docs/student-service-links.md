...
Each URL receives a deterministic automatic check before it enters the coordinator's review queue. The check accepts only direct service pages on Kafiil (`/service/<numeric-id>-<slug>`), Nafezly (the same shape) and Khamsat (`/<category>/<subcategory>/<numeric-id>-<slug>`). It supports Arabic and English slugs, stores every link over HTTPS and upgrades an `http` address on those marketplaces rather than rejecting it, removes tracking parameters and fragments, limits input length, rejects custom ports and embedded credentials, and prevents normalized duplicates. Network availability, ownership and track fit remain explicit human QC checks. The approved rules and SOPs are in `docs/service-link-launch-pack.md`.

The coordinator of the student's group reviews each slot independently. Quality tracks programme activities in a separate system and does not review service links here:

- `Locked`: the URL is accepted and becomes read-only for the student.
- `Needs Correction`: the URL remains editable and the coordinator's comment is shown above the resubmission control.
- `Pending`: the URL is waiting for the coordinator's decision and is read-only until that decision is made.
...
The review queue is available in the staff **Quality review** module to the coordinator, supervisor and Project Operations of the student's group. It has platform, track, group, coordinator, age, automatic-result and correction-count filters; 25-row pagination; SLA indicators; correction templates; Quality Lead override reasons; reviewer activity; and immutable one-decision-per-revision protection. Student 360 shows all three links and their review history. Reports include submission, approval, correction, turnaround, platform, revision, reviewer and group/coordinator metrics with CSV/XLSX export.
...
A locked link is final and a link the automatic check failed can only be returned for correction: there is no override. A student fixes the service and submits a new link instead.
