...
5. In **Sessions**, schedule the policy-approved week and 180-minute duration with an active onboarded group coach. The assigned Coach confirms it. Coach Operations handles reasoned reschedules/cancellations, monitors coverage, and reconciles every active student's attendance before delivery notes can close the session.
6. Download a roster import template. IDs are stable; existing IDs are rejected instead of silently overwritten. Import previews do not commit records.
7. Log a contact: select student, channel, timestamp and outcome; upload a PNG/JPEG screenshot; record next action, owner and due date. Completing a Contact task requires a subsequent valid contact record.
8. For controlled work: request an account, have Higher Board approve task fit and allocate an eligible account. Credential exchange is outside this pilot; never put passwords in notes or imports.
9. Progress the gig step by step with an activity screenshot. Paid gigs enter Coach Review → Coordinator L1 → Quality Review.
10. Quality accepts only after seven checks or rejects with a code and guidance. Rejection creates a coordinator correction task. EV06, EV09 or a second rejection creates an L3 case; subsequent resubmissions route to L3.
11. Inspect the Student 360 record, read-only computed graduation and audit trail. CSV/XLSX exports retain stable identifiers and exclude credential fields.
12. Use **Program flow** to govern registration, screening, admission, assessments, certificates, outcomes, Ministry withdrawal decisions and group archive. Lifecycle records are preserved rather than deleted.
13. In **Program flow → Bulk control**, use only the safe batch actions for group ownership, engagement classification and task creation. Quality approval, graduation and account allocation remain individual actions.
14. Create the Ministry report layout as a draft in **Launch readiness**. A different Project Operations or Operations Systems user must approve it before CSV/XLSX export becomes available. Every export creates a report-run record.
